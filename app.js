...
let firebaseSuppressWrite = false;
let firebaseHasWeeklyMenu = false;
const firebaseWriteQueue = {};
let firebaseWriteTimer = null;

const _lsSetItem = LS.setItem.bind(LS);
const _lsGetItem = LS.getItem.bind(LS);
const firebasePendingWrites = {};
LS.setItem = (key, value) => {
  const existing = _lsGetItem(key);
  const hadKey = existing !== null && existing !== undefined;
  _lsSetItem(key, value);
  if(firebaseKeys.has(key)){
    if(firebaseReady && !firebaseSuppressWrite){
      queueFirebaseWrite(key, value);
    } else if(!firebaseReady && hadKey){
      // preserve local changes before firebase sync completes
      firebasePendingWrites[key] = value;
    }
  }
};
...
function normalizeAduanList(list){
  const arr = coerceArray(list).filter(Boolean);
  let changed = false;
...
function normalizeCustomStudents(list){
  return coerceArray(list).filter(s=> normalizeIc(s.ic).length === 12);
}

function queueFirebaseWrite(key, value){
  if(!firebaseDb) return;
  firebaseWriteQueue[key] = parseFirebaseValue(value);
  if(firebaseWriteTimer) return;
  firebaseWriteTimer = setTimeout(()=>{
    const updates = {};
    Object.keys(firebaseWriteQueue).forEach(k=>{
      updates[k] = firebaseWriteQueue[k];
      delete firebaseWriteQueue[k];
    });
    firebaseWriteTimer = null;
    if(Object.keys(updates).length){
      firebaseDb.ref(firebaseRootPath).update(updates).catch(err=>{
        console.warn('Firebase sync gagal', err);
      });
    }
  }, 250);
}
...
function applyRemoteData(data){
  ...
  firebaseSuppressWrite = true;

  if(data.aduanList != null){
    const normalized = normalizeAduanList(data.aduanList);
    aduanList = normalized.list;
    _lsSetItem('aduanList', JSON.stringify(aduanList));
  }
  if(data.customStudents != null){
    customStudents = normalizeCustomStudents(data.customStudents);
    _lsSetItem('customStudents', JSON.stringify(customStudents));
  }
  if(data.studentDormDB != null){
    studentDormDB = coerceArray(data.studentDormDB);
    _lsSetItem('studentDormDB', JSON.stringify(studentDormDB));
  }
  if(data.menuData != null){
    _lsSetItem('menuData', JSON.stringify(data.menuData || {}));
  }
  if(data.weeklyMenuData != null){
    _lsSetItem('weeklyMenuData', JSON.stringify(coerceArray(data.weeklyMenuData)));
    firebaseHasWeeklyMenu = true;
  }
  if(data.weeklyMenuVersion != null){
    _lsSetItem('weeklyMenuVersion', String(data.weeklyMenuVersion));
  }
  if(data.weeklyMenuSeed != null){
    const seed = parseInt(data.weeklyMenuSeed, 10);
    if(!isNaN(seed)){ weeklyMenuSeed = seed; }
    _lsSetItem('weeklyMenuSeed', String(seed));
  }

  firebaseSuppressWrite = false;
  refreshUiFromData();
}
...
function seedFirebaseFromLocal(){
  ...
  firebaseKeys.forEach(key=>{
    ...
    if(raw !== null && raw !== undefined){
      payload[key] = parseFirebaseValue(raw);
    }
  });
  if(Object.keys(payload).length){
    firebaseDb.ref(firebaseRootPath).update(payload).catch(err=>{
      console.warn('Firebase seed gagal', err);
    });
  }
}
...
function startFirebaseListener(){
  if(!firebaseDb) return;
  firebaseDb.ref(firebaseRootPath).on('value', snap=>{
    const data = snap.val() || {};
    applyRemoteData(data);
  }, err=>{
    console.warn('Firebase listener gagal', err);
  });
}
...
async function initFirebaseSync(){
  ...
  if(typeof firebase === 'undefined' || !firebase.initializeApp){
    console.warn('Firebase SDK tidak tersedia.');
    return;
  }
  try{
  ...
  try{
    const snap = await firebaseDb.ref(firebaseRootPath).once('value');
    const data = snap.val();
    const pendingKeys = Object.keys(firebasePendingWrites);
    const hasPending = pendingKeys.length > 0;
    if(data){
      if(hasPending){
        const merged = {...data};
        pendingKeys.forEach(k=>{ merged[k] = parseFirebaseValue(firebasePendingWrites[k]); });
    ...
    firebaseReady = true;
    if(hasPending){
      pendingKeys.forEach(k=> queueFirebaseWrite(k, firebasePendingWrites[k]));
      pendingKeys.forEach(k=>{ delete firebasePendingWrites[k]; });
    }
...
function initWeeklyMenuStore(){
  ...
  const hasData = !!LS.getItem('weeklyMenuData');
  if(firebaseHasWeeklyMenu){
